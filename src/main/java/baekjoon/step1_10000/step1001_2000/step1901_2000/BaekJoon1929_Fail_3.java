...
/**
 * 기존 M-N 소수 구하는 방식(시간초과)
 *  - 2 ~ M전까지 나머지가 0이 안돼면 소수
 *  - M++ 변경 방식(시간초과) - 소수 리스트
 * 
 * 0번째 = 소수 2로 시작
 *  - M % 기존소수 리스트 != 0 ? 소수 : 소수 X
 *  - M++ 에라토스테네스의 체
 *  - 2부터 시작, 해당
 * 
 * 소수의 제곱부터 배수로 제거, 소수로 for문, 해당 소수 제곱시 찾을려는값보다 클 경우 break ArrayList 방식으로 변경(시간초과)
 *	- reference? 배열 사용
 *
 */
public class BaekJoon1929_Fail_3 {
