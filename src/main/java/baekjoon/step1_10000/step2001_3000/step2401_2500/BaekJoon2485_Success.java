 ...
 * 
 * 	# 가로수의 위치 값이 랜덤으로 입력된다는 가정하에 진행
 *
 */
public class BaekJoon2485_Success {
	public static void main(String[] args) throws IOException {
		...
		N_1 = arr[0]; N_2 = arr[1];
		interval_1 = N_1 > N_2 ? N_1 - N_2 : N_2 - N_1;
		
		for (int i = 2; i < arr.length; i++) {
			N_1 = N_2;
			N_2 = arr[i];
			interval_2 = N_1 > N_2 ? N_1 - N_2 : N_2 - N_1;
			if (N_1 > N_2) {
				interval_1 = gcd(interval_1, interval_2);
			} else {
				interval_1 = gcd(interval_2, interval_1);
			}
		}
		
		
		int cnt = arr[0] + interval_1;
